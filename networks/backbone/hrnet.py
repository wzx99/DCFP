...
# model_urls = {
#     'hrnetv2w48': '/gdata/wangzx/pretrained_models/hrnetv2_w48-imagenet.pth',
# }
...
class HighResolutionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.fuse_layers)):
            ...
            x_fuse.append(self.relu_inplace(y))
...
class HRNetV2(nn.Module):
    def __init__(self, align_corner, param, out_index=None, **kwargs):
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        if self.out_index is not None:
            return [x[i-1] for i in self.out_index]

        else:
            # Upsampling
            x0_h, x0_w = x[0].size(2), x[0].size(3)
            x1 = F.interpolate(
                x[1], size=(x0_h, x0_w), mode='bilinear', align_corners=ALIGN_CORNER)
            x2 = F.interpolate(
                x[2], size=(x0_h, x0_w), mode='bilinear', align_corners=ALIGN_CORNER)
            x3 = F.interpolate(
                x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=ALIGN_CORNER)
...
def build_hrnet(name, para):
    ...
    align_corner = para.get('align_corner', True)
    out_index = para.get('out_index', None)

    model = HRNetV2(align_corner, param=extra, out_index=out_index)
